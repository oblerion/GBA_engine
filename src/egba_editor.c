...
{
    int curantsprite;
    const int btnspr_size; //= 2;
    const int btncol_size; //= 26;
    Texture2D sprfond;
...
void UI_Sprite_GenSprFont()
{
    ...
    UpdateTexture(_uisprite.sprfond,img.data);
...
void UI_Sprite_Init()
{
    ...
    {
       _uisprite.btnspr[(size*y)+x] = UI_BUTTONSPRITE(
        440+(x*_uisprite.btnspr_size*16 + (_uisprite.btnspr_size*8)),
        30+(y*_uisprite.btnspr_size*16 +
        (_uisprite.btnspr_size*8)),
        _uisprite.btnspr_size,
        (size*y)+x
    );
    }
       // fond spr
    _uisprite.sprfond = LoadTextureFromImage(GenImageColor(26*16,26*16,BLACK));
...
{
    int ret=0;
    DrawTexture(_uisprite.sprfond,2,30,WHITE);
    DrawRectangleLines(440,30,16*2*16,16*2*16,StyleGetColor(1));
    for(int y=0;y<16;y++)
    for(int x=0;x<16;x++)
    {
        int id = (16*y)+x;
        if(UI_BUTTONSPRITE_Draw(_uisprite.btnspr[id]))
        {
			_uisprite.curantsprite=id;
        }
    }
    Atlas_DrawSprite(_uisprite.curantsprite,2+8*_uisprite.btncol_size,30+8*_uisprite.btncol_size,_uisprite.btncol_size,0);
    return ret;
}

void UI_Sprite_Free()
{
    UnloadTexture(_uisprite.sprfond);
}
...
char UICONFIG_Draw()
{
    if(UICONFIG_Timer())
    {
        ...
        if(Gamepad_Isplug(0))
        {
            strcpy(state_uiconfig.gamepad_name,Gamepad_GetName(0));
        }
        else if(!TextIsEqual(state_uiconfig.gamepad_name,"none"))
        {
            strcpy(state_uiconfig.gamepad_name,"none");
        }
...
int BROWSER_Draw()
{
    ...
    {
        if(!FileExists(TextFormat("./egba_manual_%s.pdf",EGBA_VERSION)))
        {
            OpenURL("https://oblerion.itch.io/gba-engine");
        }
        else
        {
            OpenURL(TextFormat("./egba_manual_%s.pdf",EGBA_VERSION));
        }
    }
...
void _StatelockEGBA(Project lproject)
{
...
void _StateEGBA(Project lproject)
{
    // default egba
...
void _StatenoEGBA(Project lproject)
{
...
void _StateDebug(Project lproject)
{
    //debug
...
void _StateNoD(Project lproject)
{
...
void _StateNoDL(Project lproject)
{
...
void _UIPROJECT_UIDefault(Project lproject)
{
                ...
                if(lproject.islua && lproject.ispng)
                {
                    if(lproject.isegba)
                    {
                        if(!_UIPROJECT.islock)
                            ...
                            _StatelockEGBA(lproject);
                    }
                    else
                    {
                        _StatenoEGBA(lproject);
                    }
                    _StateDebug(lproject);
                }
                else
                {
                    if(lproject.isegba)
...
void _UIPROJECT_UISprite(Project lproject)
{
...
void _UIPROJECT_UIRunner()
{
    if(Egba_Runner_IsDown())
    {
        if(Egba_Runner_IsDebug())
        {
            puts("[EGBA][DEBUG END]");
        }
        else
	...
	else if(_UIPROJECT.isactive)
	{
		...
		switch(_UIPROJECT.state)
		{
			case UIPROJECT_STATE_DEFAULT:
                _UIPROJECT_UIDefault(lproject);
			break;
            case UIPROJECT_STATE_HELP:
                _UIPROJECT_UIHelp();
            break;
			case UIPROJECT_STATE_VSPRITE:
                _UIPROJECT_UISprite(lproject);
			break;
...
char Egba_Editor_Init(int narg,char** sarg)
{
