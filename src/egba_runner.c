...
char _EntryPoint()
{
   const char* sentry = _GetEntryPoint();
    if(Egba_Runner_Load(
        TextFormat("%s.lua",
...
void Egba_Runner_Init(int narg,char** sarg)
{
    ...
    if(narg==2)
    {
        ...
        else if(Egba_Runner_Load(sarg[1]))
        {
        // runner init
        }
        else
